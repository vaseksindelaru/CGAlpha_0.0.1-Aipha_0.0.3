...
class BinanceWebSocketManager(BaseComponentV3):
    ...
    async def _handle_message(self, data: Dict):
        ...
        if is_depth_update and sym_lower in self.l2_buffers:
            ...
            recent_trades_1s = [
                t for t in self.last_trades if t["timestamp"] >= recent_1s_cutoff
            ]
            trade_count = len(recent_trades_1s)
            agg_buy_vol = sum(
                t["qty"] for t in recent_trades_1s if not t["is_buyer_maker"]
            )
            agg_sell_vol = sum(
                t["qty"] for t in recent_trades_1s if t["is_buyer_maker"]
            )
    ...
    def get_rolling_delta(self, symbol: str, window_seconds: float = 300) -> float:
        ...
        """
        now_ms = (
            self.last_known_binance_ts_ms
            if getattr(self, "last_known_binance_ts_ms", 0) > 0
            else time.time() * 1000
        )
        ...
        delta = sum(
            t["qty"] if not t["is_buyer_maker"] else -t["qty"] for t in recent_trades
        )
        return delta
