        ...
        self._buffer.append(
            {
                ...
                "agg_buy_vol": float(aggressive_buy_vol),
                "agg_sell_vol": float(aggressive_sell_vol),
            }
        )

    def synthesize_at_retest(
        self, t_candle_close_ms: float = None, epsilon_ms: int = 200
    ) -> dict:
        """
        Cuando el detector dispara RETEST, condensa snapshots.

        D-014 (Acoplamiento Temporal Causal): si t_candle_close_ms se provee,
        filtra snapshots con ts > (t_candle_close_ms - epsilon_ms) / 1000
        para garantizar que ninguna feature incluye información posterior
        al cierre de la vela de detección. Si el filtrado reduce el buffer
        a <10 snapshots, retorna _empty_profile() con l2_data_quality =
        "CAUSAL_REJECTED".

        Args:
            t_candle_close_ms: timestamp de cierre de la vela de detección
                               (Binance event time en ms). Si es None, no se
                               filtra (comportamiento pre-D-014).
            epsilon_ms: margen de seguridad en ms (default 200, D-014).
        """
        ...
        # D-014: Filtrado causal — excluir snapshots posteriores al cierre
        # de la vela de detección menos epsilon.
        if t_candle_close_ms is not None:
            causal_cutoff_s = (t_candle_close_ms - epsilon_ms) / 1000.0
            buf = [s for s in buf if s["ts"] <= causal_cutoff_s]
            if len(buf) < 10:
                return self._causal_rejected_profile()
        ...
        latest = buf[-1]
        dr_current = (
            float(latest["obi_1"] / latest["obi_10"]) if latest["obi_10"] != 0 else 1.0
        )

        w5_dr = (
            np.mean(
                [(s["obi_1"] / s["obi_10"]) if s["obi_10"] != 0 else 1.0 for s in w5]
            )
            if w5
            else dr_current
        )
        ...
        return {
            ...
            "obi_10_min_30s": float(np.min(obi_10_series))
            if len(obi_10_series) > 0
            else 0.0,
            "obi_10_max_30s": float(np.max(obi_10_series))
            if len(obi_10_series) > 0
            else 0.0,
            "obi_10_std_30s": float(np.std(obi_10_series))
            if len(obi_10_series) > 0
            else 0.0,
        ...
        return {
            ...
            **{
                ...
                for k in [
                    ...
                    "delta_acceleration_15s",
                    "depth_ratio_1_10",
                    "depth_ratio_1_10_gradient_5s",
                    "trade_intensity_5s",
                    "trade_intensity_15s",
                    "aggressive_buy_pct_5s",
                    "aggressive_buy_pct_15s",
                ]
            },
        }

    def _causal_rejected_profile(self) -> dict:
        """Profile retornado cuando D-014 filtra todos los snapshots
        por estar posteriores al cierre de la vela de detección."""
        return {
            "window_seconds": 30,
            "n_snapshots": 0,
            "l2_data_quality": "CAUSAL_REJECTED",
            **{
                k: 0.0
                for k in [
                    "obi_10_gradient_5s",
