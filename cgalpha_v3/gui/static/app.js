...
const ULTRA_LIGHT_MODE = UI_MODE_QUERY === "normal" ? false : true; // Ultraligero por defecto.

try {
  localStorage.setItem(
    "cgalpha_ui_mode",
    ULTRA_LIGHT_MODE ? "ultralight" : "normal",
  );
} catch (e) {
...
async function fetchStrategyStatus() {
  ...
  try {
    ...
    view.innerHTML = `
            ...
            ${data.components
              .map(
                (
                  c,
                ) => `<div style="background:#0f1b2d; padding:8px; border-radius:6px; margin-bottom:5px; border:1px solid rgba(255,255,255,0.05); display:flex; justify-content:space-between; align-items:center;">
    ...
    view.innerHTML = "<p>Error syncing pipeline v3.</p>";
  }
}

async function executePipelineCycle() {
  if (!confirm("¿Deseas iniciar un Ciclo Masivo de Cosecha y Entrenamiento?"))
    return;
  try {
    const resp = await apiFetch("/api/lila/execute-cycle", {
      method: "POST",
...
async function fetchVaultStatus() {
  try {
    ...
    // Zona 1: Oracle
    document.getElementById("vault-oracle-status").innerText =
      data.oracle.status;
    document.getElementById("vault-oracle-samples").innerText =
      data.oracle.samples;
    document.getElementById("vault-oracle-acc").innerText =
      data.oracle.test_accuracy.toFixed(4);
    document.getElementById("vault-sig-warning").style.display = data.oracle
      .is_significant
      ? "none"
      : "inline-block";

    // Zona 2: Market
    document.getElementById("vault-mkt-retests").innerText =
      data.market.retests_detected;
    document.getElementById("vault-mkt-trades").innerText =
      data.market.total_trades;
    document.getElementById("vault-mkt-real-trades").innerText =
      data.market.real_trades;
    document.getElementById("vault-mkt-placeholder-trades").innerText =
      data.market.placeholder_trades;

    // Zona 3: Evolution
    document.getElementById("vault-evo-pending").innerText =
      data.evolution.pending;
    document.getElementById("vault-evo-rejected").innerText =
      data.evolution.rejected_safety;
    document.getElementById("vault-evo-applied").innerText =
      data.evolution.applied;
  } catch (err) {
...
async function fetchEvolutionHeartbeat() {
  try {
    ...
    console.warn("Evolution heartbeat error:", e);
  }
}
...
function updateEvolutionHeartbeat(d) {
  ...
  statusPill.className =
    "pill " + (status === "ok" ? "pill-running" : "pill-idle");
  ...
  if (oracleEl) {
    ...
    oracleEl.style.color = oracleTrained
      ? "var(--accent)"
      : "var(--text-muted)";
  }
...
function updateMarketLive(d) {
  ...
  setText(
    "mkt-ts",
    `${(dc * 100).toFixed(2)}% (${dc <= 0.25 ? "STABLE" : "SHIFTED"})`,
  );
  ...
  if (badge) {
    ...
    badge.className =
      "pill " + (dq === "valid" ? "pill-idle" : "pill-degraded");
  }
}
...
async function fetchLiveSignals() {
  try {
    ...
    console.warn("Live signals error:", e);
  }
}
...
function renderLiveSignals(signals) {
  ...
  if (!signals || !signals.length) {
    el.innerHTML =
      '<div style="font-size:11px; color:var(--text-dim); text-align:center; padding:10px;">Esperando señales del ShadowTrader...</div>';
    return;
  }
...
function renderPortfolio(d) {
  ...
  if (balEl) {
    ...
    balEl.style.color =
      d.balance >= d.initial_balance ? "var(--accent)" : "var(--red)";
  }
...
function updatePnLChart(balance) {
  ...
  if (!pnlChart) {
    ...
    ds.backgroundColor =
      balance >= 10000 ? "rgba(0, 242, 255, 0.1)" : "rgba(255, 68, 68, 0.1)";
...
function renderExposure(exposure) {
  ...
  if (!entries.length) {
    el.innerHTML =
      '<div style="font-size:10px; color:var(--text-dim);">No hay exposición activa.</div>';
    return;
  }
...
async function confirmPanicClose() {
  if (
    !confirm(
      "⚠️ ¿ESTÁS SEGURO? Se cerrarán TODAS las posiciones de ETH y BTC inmediatamente.",
    )
  )
    return;
  try {
    ...
    alert("Error en Panic Close: " + e.message);
  }
}
...
function renderSessionSummary(d) {
  ...
  if (history.length === 0) {
    wrEl.innerText = "0%";
    listEl.innerHTML =
      '<div style="opacity:0.5;">No hay trades cerrados aún.</div>';
    return;
  }
...
function renderEvents(events) {
  ...
  if (!events || !events.length) {
    el.innerHTML =
      '<span style="color:var(--text-muted);font-size:12px">Sin eventos aún.</span>';
    return;
  }
...
function renderRollbacks(snaps) {
  ...
  if (!snaps || !snaps.length) {
    el.innerHTML =
      '<span style="font-size:10px;color:var(--text-muted)">Sin snapshots.</span>';
    return;
  }
  el.innerHTML = snaps
    .slice(0, 5)
...
async function doRollback(path) {
  if (
    !confirm(
      "¿Confirmar restauración de snapshot? Se perderán configuraciones actuales.",
    )
  )
    return;
  try {
...
function updateLibraryStatus(status) {
  ...
  setText(
    "lib-last-ingestion",
    status.last_ingestion ? formatTs(status.last_ingestion) : "—",
  );
  ...
  try {
    ...
    if (selectedLibrarySourceId) {
      const found = libraryItems.find(
        (s) => s.source_id === selectedLibrarySourceId,
      );
    ...
    if (el)
      el.innerHTML =
        '<span style="color:var(--text-muted);font-size:12px">Error cargando biblioteca.</span>';
  }
}
...
function renderLibraryResults(items) {
  ...
  if (!items || !items.length) {
    el.innerHTML =
      '<span style="color:var(--text-muted);font-size:12px">Sin resultados.</span>';
    return;
  }
  el.innerHTML = items
    .map(
...
async function selectLibrarySource(sourceId) {
  ...
  try {
    const source = await apiFetch(
      `/api/library/sources/${encodeURIComponent(sourceId)}`,
    );
...
function renderTheoryRecentSources(items) {
  ...
  if (!items.length) {
    el.innerHTML =
      '<span style="color:var(--text-muted);font-size:12px">Sin fuentes recientes.</span>';
    return;
  }
  el.innerHTML = items
    .map(
...
async function validateTheoryClaim() {
  ...
  const sourceIdsRaw = (
    document.getElementById("th-source-ids")?.value || ""
  ).trim();
...
async function fetchAdaptiveBacklog() {
  try {
    ...
    if (el)
      el.innerHTML =
        '<span style="color:var(--text-muted);font-size:12px">Error cargando backlog.</span>';
  }
}
...
function renderAdaptiveBacklog(items) {
  ...
  if (!items.length) {
    el.innerHTML =
      '<span style="color:var(--text-muted);font-size:12px">Sin items abiertos.</span>';
    return;
  }
  el.innerHTML = items
    .map(
...
async function resolveBacklogItem(itemId) {
  ...
  try {
    await apiFetch(`/api/lila/backlog/${encodeURIComponent(itemId)}/resolve`, {
      method: "POST",
      body: JSON.stringify({
        resolution_note: "Resuelto desde GUI Theory Live",
      }),
    });
    refreshTheoryLive();
  } catch (e) {
...
function renderExperimentWindows(items) {
  ...
  if (!items.length) {
    el.innerHTML =
      '<span style="color:var(--text-muted);font-size:12px">Sin resultados aún.</span>';
    return;
  }
  el.innerHTML = items
    .map(
...
function renderApproachHistogram(hist) {
  ...
  if (!entries.length || total === 0) {
    el.innerHTML =
      '<span style="color:var(--text-muted);font-size:12px">Sin histograma aún.</span>';
    return;
  }
...
async function proposeExperiment() {
  ...
  const approaches = (
    document.getElementById("exp-approaches")?.value || ""
  ).trim();
  if (!hypothesis) {
...
async function ingestLearningMemory() {
  ...
  const field = (
    document.getElementById("learn-field")?.value || "memory_librarian"
  ).trim();
  if (!content) {
...
async function runLearningRegimeCheck() {
  ...
  try {
    ...
    const msg = document.getElementById("learn-msg");
    if (msg) {
      msg.textContent = shift
        ? "✓ Regime shift detectado"
  ...
  try {
    ...
    const sourceLabel =
      source === "disk_fallback"
        ? "disco (collector externo)"
        : "memoria local";
    setText(
      "forensics-pending-note",
      `Retests esperando resolución · fuente: ${sourceLabel}`,
    );
...
function renderForensicsDistribution(dist) {
  ...
  if (total === 0) {
    el.innerHTML =
      '<div style="font-size:11px; color:var(--text-muted); text-align:center;">Sin datos aún</div>';
    return;
  }
...
function renderForensicsActiveZones(zones) {
  ...
  if (!zones || !zones.length) {
    el.innerHTML =
      '<div class="placeholder"><span class="ph-icon">🔍</span>Sin zonas activas reportadas.<br>Esperando que el detector inicie o encuentre zonas.</div>';
    return;
  }
  ...
  el.innerHTML = zones
    .map((z) => {
      const effectiveDir = z.effective_direction || z.direction;
      const dirColor =
        effectiveDir === "bullish" ? "var(--accent)" : "var(--red)";
      ...
      const flipBadge = z.polarity_flipped
        ? `<span style="font-size:9px; color:var(--text-dim); border:1px solid var(--text-dim); padding:1px 4px; border-radius:3px; margin-left:6px;">FLIP</span>`
        : "";
      const origDir =
        z.polarity_flipped && z.original_direction
          ? `<span style="font-size:9px; color:var(--text-muted); margin-left:6px;">orig: ${escHtml(z.original_direction.toUpperCase())}</span>`
          : "";
      ...
      return `<div style="background:var(--bg3); padding:12px; border-radius:10px; border:1px solid var(--border); margin-bottom:8px;">
            ...
            <div style="display:flex; gap:16px; font-size:11px; margin-bottom:6px;">
                ...
                <div style="margin-left:auto; display:flex; align-items:center;">
                    <span style="color:${dirColor}; font-weight:700;">${escHtml(effectiveDir ? effectiveDir.toUpperCase() : "")}</span>
                    ${flipBadge}${origDir}
                </div>
            </div>
...
function renderForensicsMonitors(monitors) {
  ...
  if (!monitors.length) {
    el.innerHTML =
      '<div class="placeholder"><span class="ph-icon">🔇</span>Sin retests pendientes.<br>El sistema está esperando un toque de zona.</div>';
    return;
  }
  el.innerHTML = monitors
    .map((m) => {
      const progress =
        m.lookahead_bars > 0
          ? Math.min((m.bars_elapsed / m.lookahead_bars) * 100, 100)
          : 0;
      const dirColor =
        m.direction === "bullish" ? "var(--accent)" : "var(--red)";
...
function renderForensicsTimeline(entries) {
  ...
  if (!entries.length) {
    el.innerHTML =
      '<div class="placeholder"><span class="ph-icon">📭</span>Sin samples resueltos aún.<br>Los resultados aparecerán aquí tras la resolución.</div>';
    return;
  }
  const labelColors = {
    BOUNCE_STRONG: {
...
async function openForensicsSnapshot(sampleId) {
  ...
  try {
    const snap = await apiFetch(
      `/api/l2-forensics/snapshot/${encodeURIComponent(sampleId)}`,
    );
...
async function ingestLibrarySource() {
  ...
  try {
    ...
    if (msg) {
      msg.textContent = res.is_new
        ? "✓ Fuente ingestada"
        : "✓ Fuente duplicada detectada";
      msg.classList.remove("hidden");
      setTimeout(() => msg.classList.add("hidden"), 2500);
    }
...
function saveRiskParams() {
  ...
  const maxPosition = parseFloat(
    document.getElementById("risk-max-position").value,
  );
  const maxSignals = parseInt(
    document.getElementById("risk-max-signals").value,
    10,
  );
  const minQuality = parseFloat(
    document.getElementById("risk-min-quality").value,
  );
...
function startNewLilaChat() {
  if (
    !confirm(
      "Start a new conversation with Lila? Current messages will be saved to history.",
    )
  )
    return;
  ...
  if (msgs.children.length > 1) {
    ...
    localStorage.setItem(
      "lila_history",
      JSON.stringify(lilaHistory.slice(0, 10)),
    );
  }
...
function renderLilaHistory() {
  ...
  list.innerHTML = lilaHistory.length
    ? ""
    : '<div style="color:var(--text-muted); font-size:12px; margin-top:20px; text-align:center;">No history found.</div>';
...
async function sendLilaMessage() {
  ...
  try {
    ...
    appendChatMessage(
      "lila",
      "Internal Error: Connection to Lila Core V3 failed.",
    );
  }
}
...
└─────────────────────────────────┘</pre>
  ...
  },
  {
    ...
    items: [
      {
        q: "¿Qué es la Triple Coincidence Strategy?",
        a: "Es la estrategia fundacional de CGAlpha v3. Detecta zonas de alta probabilidad cuando tres señales independientes convergen: vela clave (absorción institucional), zona de acumulación (consolidación + volumen), y mini-tendencia (ZigZag con R² > 0.45). A diferencia de la v2, NO opera en la detección sino que espera el retest y captura features de microestructura en ESE momento.",
      },
      {
        q: "¿Por qué las features se capturan EN el retest y no en la detección?",
        a: "Porque las condiciones del mercado en el momento del retest son más predictivas que las condiciones en la detección. La zona puede haberse detectado hace 30 velas, pero el estado del order book (OBI), el delta acumulado y el VWAP EN el retest reflejan la presión actual del mercado, que es lo que determina si el precio rebotará (BOUNCE) o romperá (BREAKOUT).",
      },
      {
        q: "¿Qué es el Oracle y cómo funciona?",
        a: "El OracleTrainer_v3 implementa Meta-Labeling: un modelo ML secundario que predice si una señal del detector primario será exitosa. Se entrena con pares {features_retest → outcome}. Su predicción (confidence score) filtra señales: solo con confidence > 0.70 se opera. El modelo se re-entrena incrementalmente con cada nuevo retest observado.",
      },
      {
        q: "¿Qué es el Temporal Leakage?",
        a: "Error donde el algoritmo usa datos del futuro. CGAlpha tiene gates matemáticos en cada ExperimentRunner que bloquean cualquier experimento con leakage. Si train_end >= oos_start, el experimento es rechazado automáticamente con error: temporal_leakage.",
      },
      {
        q: "¿Cómo funciona la memoria de 5 niveles?",
        a: "Nivel 0a (Meta-Cognitivo/Principios, TTL 24h) → 0b (Papers, TTL 7d) → 1 (Almacenamiento operacional, TTL 30d) → 2 (Recuperación semántica, TTL 90d) → 3 (Playbooks, permanente, aprobación humana) → 4 (Estrategia/ADN, permanente, requiere NexusGate + Sharpe > 1.5).",
      },
      {
        q: "¿Cómo funciona el Experiment Loop?",
        a: 'Crea una Proposal (hipótesis + approach_types), ejecútala con "Run Experiment". El sistema hace Walk-Forward validation (≥3 ventanas temporal) calculando métricas netas post-fricción (fees + slippage). Solo con sharpe > 1.5, human_approval y ΔCausal > 0 se puede promover a nivel 4 (STRATEGY).',
      },
      {
        q: "¿Puedo operar en modo live?",
        a: "No en Fase 0. La GUI actual es Control Room con datos mock (precios simulados). El pipeline está diseñado para generar dataset de entrenamiento del Oracle via backtesting intensivo. La conexión WebSocket a Binance para datos reales está planificada para fases posteriores.",
      },
      {
        q: "¿Qué es el NexusGate?",
        a: "El NexusGate es el árbitro final del pipeline. Su gate es binario: PROMOTE_TO_LAYER_2 o REJECT. Condiciones: ΔCausal > 0 (mejora causal demostrada en OOS), blind_test_ratio ≤ 0.25, test_coverage ≥ 0.80, oos_hit_rate_improvement > 0, y human_approval == True.",
      },
      {
        q: "¿Cómo se evoluciona la estrategia sin predefinir los pasos?",
        a: "El ciclo evolutivo es emergente: (1) ejecutar estrategia → (2) analizar datos reales → (3) AutoProposer detecta drift y propone mejora → (4) Lila busca soporte teórico en Library → (5) si hay gap, genera backlog para buscar fuentes → (6) se valida con Walk-Forward → (7) el resultado genera la siguiente propuesta. Cada paso nace del anterior, no de un plan predefinido.",
      },
      {
        q: '¿Qué es "primary_source_gap"?',
        a: "Aparece cuando intentas validar un claim (hipótesis) pero no hay fuentes primary (peer-reviewed, ev_level=1) en la Library que lo respalden. Solución: ingestar al menos una fuente primary. Esto asegura que las decisiones del sistema tienen base científica verificable.",
      },
      {
        q: "¿Cómo hago rollback si algo sale mal?",
        a: 'En Mission Control se listan los snapshots disponibles. Selecciona uno y pulsa "Restaurar". Esto recupera configuración, memoria y estado de experimentos al momento del snapshot.',
      },
      {
        q: "¿Qué son los 7 componentes del ADN Permanente?",
        a: "BinanceVisionFetcher_v3 (datos), TripleCoincidenceDetector (detección + retest), ZonePhysicsMonitor_v3 (física del retest), ShadowTrader (posiciones virtuales), OracleTrainer_v3 (Meta-Labeling), NexusGate (gate binario), AutoProposer (drift + mejoras). Cada componente tiene su origen en la Bóveda legacy y ha sido adaptado y validado para v3.",
      },
    ],
  },
];
...
function filterHelp() {
  ...
  helpDebounceTimer = setTimeout(() => {
    ...
    const matches = HELP_DATA.filter(
      (d) =>
...
async function fetchTrainingReviewData() {
  try {
    ...
    setText(
      "tr-bounce-pct",
      (data.outcome_distribution?.bounce_pct || 0) + "%",
    );
...
function getFilteredRetests() {
  ...
  return trainingData.retests.filter((rt) => {
    const matchOutcome =
      trainingCurrentFilter === "all" || rt.outcome === trainingCurrentFilter;
    const matchRegime =
      trainingCurrentRegime === "all" || rt.regime === trainingCurrentRegime;
...
function renderTrainingChart() {
  ...
  if (!ohlcv.length) {
    chartDiv.innerHTML =
      '<div style="display:flex; align-items:center; justify-content:center; height:100%; color:var(--text-muted); font-size:14px;">No hay datos OHLCV disponibles</div>';
    return;
  }
  ...
  if (trainingViewMode === "context" && trainingSelectedZone) {
    ...
    if (zone) {
      ...
      const endIdx = Math.min(
        ohlcv.length - 1,
        zone.zone_end_idx + trainingContextPadding,
      );
  ...
  function priceToY(price) {
    return (
      marginTop +
      chartHeight -
      (chartHeight * (price - minPrice)) / adjustedRange
    );
  }
    ...
    zone.retest_indices.forEach((rtIdx) => {
      ...
      if (rtData.outcome) {
        ...
        if (outcomeX > 0 && outcomeX < width) {
          const outcomeColor =
            rtData.outcome === "BOUNCE" ? "#00d4aa" : "#ff6b6b";
  ...
  if (trainingSelectedZone) {
    ...
    if (zone) {
      ...
      if (
        ...
        const borderColor =
          zDir === "bullish" || zDir === "long" ? "#00d4aa" : "#ff6b6b";
