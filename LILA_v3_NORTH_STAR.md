...
[1] BinanceVisionFetcher          ← Ingestión robusta de datos OHLCV + Microestructura
         ↓
[2] AbsorptionCandleDetector      ← Triple Coincidencia: Key Candles como zonas de liquidez
         ↓
[3] ZonePhysicsMonitor            ← Re-test activo: ¿Absorción o Ruptura?
         ↓
[4] ShadowTrader                  ← Posiciones ficticias → captura de trayectorias MFE/MAE
         ↓
[5] OracleTrainer (Meta-Labeling) ← Entrena Oracle v3 recursivamente con resultados reales
         ↓
...
| `signal_detection_lab.py` (Triple Coincidencia) | `AbsorptionCandleDetector_v3` | 🔴 CRÍTICA | IN_REVIEW |
...
**AbsorptionCandleDetector_v3**  
Componente v3 que hereda del `KeyCandleDetector` y `SignalDetectionLab` de Capa 1. Detecta velas de absorción institucional: alto volumen + cuerpo pequeño + pabilos prominentes. Funciona en 1–5 minutos. Parámetros validados: `volume_percentile_threshold=80`, `body_percentile_threshold=30`, `ema_period=200`. Primer componente de la Simple Foundation Strategy.
...
class ComponentManifest:
    ...
    outputs: List[str]         # ["zone_state: str", "absorption_score: float"]
...
class ZonePhysicsMonitor_v3(BaseComponentV3):
    ...
    ║    - Salida enriquecida con absorption_score (0-1)   ║
...
║                                                               ║
║  cgalpha vault search "zone re-test absorption detection"     ║
...
cgalpha vault harvest --component signal_detection_lab.py --target AbsorptionCandleDetector_v3
